    ...
    <footer className="flex flex-wrap justify-center md:justify-between overflow-hidden gap-10 md:gap-20 mt-40 py-6 px-6 md:px-16 lg:px-24 xl:px-32 text-[13px] text-gray-500">
        ...
        {footerData.map((section, index) => (
          <div key={index}>
            ...
            <ul className="mt-2 space-y-2">
              {section.links.map(({ href, name }: ILink, index: number) => (
                <li key={index}>
                  <Link to={href} className="hover:text-orange-600 transition">
                    {name}
                  </Link>
                </li>
              ))}
      ...
      >
        ...
        <p className="mt-3 text-center">
          &copy; {new Date().getFullYear()} <Link to="#Thumbora">Thumbora</Link>
        </p>
      </motion.div>
    </footer>
  );
